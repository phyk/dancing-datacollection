...
pub const SELECTOR_EVENT_NAME: &str = ".eventhead td";
pub const SELECTOR_EVENT_DATE: &str = ".comphead";
pub const SELECTOR_ORGANIZER: &str = ".organizer";
pub const SELECTOR_HOSTING_CLUB: &str = ".hosting-club";
pub const SELECTOR_COMPETITION_ITEM: &str = "center a, .pro_p_zeile a, .t_zeile a";
pub const SELECTOR_COMPETITION_TITLE: &str = ".compbutton, b";
pub const SELECTOR_PARTICIPANT_ROW: &str = "table.tab1 tr, table.tab2 tr";
pub const SELECTOR_PARTICIPANT_CELL_RANK: &str = "td.td3r, td.td3c";
pub const SELECTOR_PARTICIPANT_CELL_DATA: &str = "td.td2c, td.td5, td.td6";
pub const SELECTOR_OFFICIAL_ROW: &str = "table.tab1 tr";
pub const SELECTOR_OFFICIAL_CELL_ROLE: &str = "td.td2, td.td2r";
pub const SELECTOR_OFFICIAL_CELL_DATA: &str = "td.td5";
...
pub fn parse_participants(html: &str) -> Result<Vec<Participant>, ParsingError> {
    let fragment = Html::parse_document(html);
    let row_sel = Selector::parse(SELECTOR_PARTICIPANT_ROW).unwrap();
    let rank_sel = Selector::parse(SELECTOR_PARTICIPANT_CELL_RANK).unwrap();
    let data_sel = Selector::parse(SELECTOR_PARTICIPANT_CELL_DATA).unwrap();
...
pub fn parse_officials(html: &str) -> Result<Officials, ParsingError> {
    let fragment = Html::parse_document(html);
    let row_sel = Selector::parse(SELECTOR_OFFICIAL_ROW).unwrap();
    let role_sel = Selector::parse(SELECTOR_OFFICIAL_CELL_ROLE).unwrap();
    let data_sel = Selector::parse(SELECTOR_OFFICIAL_CELL_DATA).unwrap();
...
pub fn parse_ergwert(html: &str, dances: &[Dance]) -> (Vec<(String, BTreeMap<String, BTreeMap<String, BTreeMap<Dance, bool>>>)>, Vec<(String, BTreeMap<String, BTreeMap<String, BTreeMap<Dance, u32>>>)>) {
...
pub fn parse_event_index(html: &str) -> Result<Competition, ParsingError> {
    ...
    if competitions.is_empty() {
        if let Some(ref name) = event_name {
            if let Ok(mut comp) = parse_competition_from_title(name) {
                comp.organizer = organizer.clone();
                comp.hosting_club = hosting_club.clone();
                if comp.date.is_none() { comp.date = event_date; }
                competitions.push(comp);
        ...
        if competitions.is_empty() && !title.is_empty() {
            if let Ok(mut comp) = parse_competition_from_title(&title) {
                comp.organizer = organizer.clone();
                comp.hosting_club = hosting_club.clone();
                if comp.date.is_none() { comp.date = event_date; }
                competitions.push(comp);
...
pub fn extract_event_data(data_dir: &str) -> Result<Competition> {
    ...
    let mut comp = if let Some(ref html) = index_html {
        match parse_event_index(html) {
            Ok(c) => c,
            Err(_) if erg_html.is_some() => parse_event_index(erg_html.as_ref().unwrap()).map_err(|e| anyhow::anyhow!("Parsing error: {}", e))?,
            Err(e) => return Err(anyhow::anyhow!("Parsing error: {}", e)),
        }
    } else if let Some(ref html) = erg_html {
        parse_event_index(html).map_err(|e| anyhow::anyhow!("Parsing error: {}", e))?
    } else {
    ...
    for file in files {
        ...
        if p.exists() {
            if let Ok(content) = fs::read_to_string(&p) {
                match file {
                    "erg.htm" => {
                        if let Ok(parts) = parse_participants(&content) {
                            comp.participants = parts;
                        }
                        let detected_dances = parse_dances_from_table(&content);
                    ...
                    "deck.htm" => {
                        if let Ok(off) = parse_officials(&content) {
                            comp.officials = off;
                        }
...
mod tests {
    ...
    #[test]
    fn test_parse_date() {
        assert_eq!(parse_date("11.05.2024"), Some(NaiveDate::from_ymd_opt(2024, 5, 11).unwrap()));
        assert_eq!(parse_date("05/Jul/2025"), Some(NaiveDate::from_ymd_opt(2025, 7, 5).unwrap()));
        assert_eq!(parse_date("17. Mai 2025"), Some(NaiveDate::from_ymd_opt(2025, 5, 17).unwrap()));
    }

    #[test]
    fn test_parse_competition_from_title() {
        let comp = parse_competition_from_title("11.05.2024 Hgr.II D Standard").unwrap();
    ...
    fn test_parse_tabges_vertical() {
        ...
        let crosses = parse_tabges(html, &dances);
    ...
    fn test_min_dances_2026_compliance() {
        let comp2024 = parse_competition_from_title("11.05.2024 Hgr.II D Standard").unwrap();
        assert_eq!(comp2024.min_dances, 3);

        let comp2026 = parse_competition_from_title("11.05.2026 Hgr.II D Standard").unwrap();
    ...
    fn test_parse_participants() {
        ...
        "#;
        let participants = parse_participants(html).unwrap();
    ...
    fn test_parse_officials() {
        ...
        "#;
        let officials = parse_officials(html).unwrap();
    ...
    fn test_real_wdsf_world_open_tabges() {
        ...
        let dances = vec![Dance::Samba];
        let results = parse_tabges(&html, &dances);
    ...
    fn test_real_wdsf_rising_stars_ergwert() {
        ...
        let results = parse_ergwert(&html, &dances);
