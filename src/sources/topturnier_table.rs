use crate::models::{Dance, Officials, Round, RoundData, WDSFScore};
use regex::Regex;
...
static RE_SCORE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(crate::assets::PATTERN_SCORE).unwrap());
static RE_BIB_PARENS: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(crate::assets::PATTERN_BIB_PARENS).unwrap());
        ...
        for (r_idx, tr) in table.select(&tr_sel).enumerate() {
            ...
            for i in 0..cells.len() {
                ...
                let is_phantom = td
                    .value()
                    .attr("class")
                    .is_some_and(|c| c.split_whitespace().any(|cls| cls == "td2ww"));
                ...
                if td
                    .value()
                    .attr("class")
                    .is_some_and(|c| c.split_whitespace().any(|cls| cls == "td2w"))
                    && i + 1 < cells.len()
                {
    ...
    for node in el.children() {
        if let Some(element) = ElementRef::wrap(node) {
            ...
            if name == "br" {
                ...
                .value()
                .attr("class")
                .is_some_and(|c| c.contains("tooltip") && !c.contains("tooltipnar"))
            {
                continue;
            } else {
...
pub fn identify_columns(grid: &TableGrid) -> Vec<ColumnType> {
    ...
    for c in 0..grid.width {
        if col_types[c] == ColumnType::Unknown {
            ...
            if !has_bib_col
                && grid.height > row_to_check
                && RE_BIB_PARENS.is_match(&grid.rows[row_to_check][c])
            {
                col_types[c] = ColumnType::Participant;
            }
...
fn extract_horizontal(grid: &TableGrid) -> Vec<IntermediateResult> {
    ...
    let participant_idx = col_types
        .iter()
        .position(|t| matches!(t, ColumnType::Participant));
    let rank_idx = col_types.iter().position(|t| matches!(t, ColumnType::Rank));
    let round_idx = col_types
        .iter()
        .position(|t| matches!(t, ColumnType::Round));

    if bib_idx.is_none() && participant_idx.is_none() {
        return results;
    }

    let start_row = (0..grid.height)
        .find(|&r| {
            ...
            match col_type {
                ColumnType::Mark { dance, judge } => {
                    ...
                    for (i, round_name) in round_vals.iter().enumerate() {
                        ...
                        marks_by_round.entry(round_name.clone()).or_default().push(
                            IntermediateMark {
                                dance: *dance,
...
pub fn to_rounds(
    intermediate: Vec<IntermediateResult>,
    dances: &[Dance],
    officials: &Officials,
) -> Vec<Round> {
                ...
                match &mut round.data {
                            ...
                            && mark.value.chars().all(|c| c.is_ascii_digit())
                        {
                            for (j_idx, ch) in mark.value.chars().enumerate() {
                                if let Some(official_judge) = officials.judges.get(j_idx) {
                                    marking_crosses
                                        .entry(official_judge.code.clone())
                                        .or_default()
                                        .entry(res.bib)
                                        .or_default()
                    ...
                    RoundData::DTV { dtv_ranks } => {
                        if let Some(judge) = mark.judge {
                            ...
                            } else if let Ok(total_rank) =
                                mark.value.replace(',', ".").parse::<f64>()
                            {
    ...
    rounds.sort_by_key(|r| {
        let count = match &r.data {
            ...
            RoundData::WDSF { wdsf_scores } => {
                wdsf_scores.values().next().map(|m| m.len()).unwrap_or(0)
            }
        };
