    ...
    if table:
        for cell in table.find_all('td', class_='td3r'):
            ...
            if participant.name_one and participant.number is not None:
                ...
                print(f'Invalid participant skipped (wert_er): name_one={participant.name_one}, number={participant.number}', flush=True)
    return participants 
...
def extract_judges_from_wert_er(soup):
    ...
    for cell in judge_cells:
        ...
        span = cell.find('span')
        if span:
            code = cell.contents[0].strip() if cell.contents and isinstance(cell.contents[0], str) else ''
